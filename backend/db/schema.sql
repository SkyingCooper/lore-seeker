...
COMMENT ON COLUMN search_tasks.user_id       IS '发起搜索的用户 ID（关联 users.id）';
COMMENT ON COLUMN search_tasks.topic_id      IS '关联主题 ID（关联 topics.id），主题删除后置 NULL';
...
COMMENT ON COLUMN search_tasks.updated_at    IS '最后更新时间（状态变更、评分写入），由触发器自动维护';
...
COMMENT ON COLUMN reports.task_id    IS '关联搜索任务 ID（关联 search_tasks.id），唯一约束保证一任务一报告';
...
COMMENT ON COLUMN search_histories.task_id    IS '关联搜索任务 ID（关联 search_tasks.id），唯一约束保证一任务一条历史';
COMMENT ON COLUMN search_histories.topic_id   IS '冗余存储关联主题 ID（关联 topics.id），加速按主题过滤';
COMMENT ON COLUMN search_histories.report_id  IS '关联知识文档 ID（关联 reports.id），任务完成后由 worker 回填';
...
COMMENT ON COLUMN working_sessions.id           IS '工作记忆归档主键，自增';
COMMENT ON COLUMN working_sessions.user_id      IS '所属用户 ID（关联 users.id）';
COMMENT ON COLUMN working_sessions.task_id      IS '关联搜索任务 ID（关联 search_tasks.id）';
...
COMMENT ON COLUMN episodic_logs.id          IS '情景日志主键，自增';
COMMENT ON COLUMN episodic_logs.user_id     IS '所属用户 ID（关联 users.id）';
COMMENT ON COLUMN episodic_logs.task_id     IS '关联搜索任务 ID（关联 search_tasks.id）';
COMMENT ON COLUMN episodic_logs.session_key IS '关联工作记忆的 session_key';
COMMENT ON COLUMN episodic_logs.event_type  IS '事件类型：conversation（对话）| task_run（任务执行）| search（搜索动作）| error（异常）';
COMMENT ON COLUMN episodic_logs.content     IS '事件完整内容';
...
COMMENT ON COLUMN semantic_memories.id          IS '语义记忆主键，自增';
COMMENT ON COLUMN semantic_memories.user_id     IS '所属用户 ID（关联 users.id）；NULL 表示全局记忆';
...
COMMENT ON COLUMN semantic_memories.source_id   IS '来源记录 ID，与 source_type 配合使用，可为空';
...
COMMENT ON COLUMN user_preferences.id         IS '偏好记录主键，自增';
COMMENT ON COLUMN user_preferences.user_id    IS '所属用户 ID（关联 users.id）';
...
-- 同一用户不允许重复 key
CREATE UNIQUE INDEX uq_user_preferences_user_key ON user_preferences(user_id, key);
...
COMMENT ON COLUMN skill_memories.id               IS 'Skill 记忆主键，自增';
COMMENT ON COLUMN skill_memories.title            IS '一级：关键词/标题，用于快速匹配和索引';
COMMENT ON COLUMN skill_memories.content          IS '二级：完整 SOP 步骤内容，命中后加载';
COMMENT ON COLUMN skill_memories.citation         IS '三级：引用与解释，含来源说明、边界条件、例外情况';
COMMENT ON COLUMN skill_memories.scope            IS '作用范围：global（系统内置）| user（用户私有）';
COMMENT ON COLUMN skill_memories.user_id          IS 'scope=user 时关联的用户 ID（关联 users.id）';
COMMENT ON COLUMN skill_memories.trigger_patterns IS '触发匹配关键词列表，格式 ["关键词1", "关键词2"]';
...
CREATE INDEX idx_episodic_logs_event_type   ON episodic_logs(event_type);
...
CREATE INDEX idx_episodic_logs_user_time        ON episodic_logs(user_id, created_at DESC);
