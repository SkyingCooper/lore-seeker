...
- 对特定域名，如果 `tool_mcp.crawler.source_capabilities.<domain>.api_tool` 已声明，则优先走站点专属 API Tool。
- 搜索型 MCP 不默认压过本地 API Tool；站点型 MCP 只有在域名级显式声明时才允许前置。
- 所有结果按 URL 去重。
...
3.3.1 如果站点存在专属 API Tool 或站点型 MCP，优先使用该结构化入口。
3.3.2 如果没有结构化入口但已配置 RSS，优先 RSS。
3.3.3 如果都没有，再进入网页抓取链路。
...
2.4.1 crawler 链路顺序固定为：RSS -> HTTP 静态抓取 -> 命中规则后降级 Playwright。
